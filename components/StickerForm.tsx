...
const StickerForm: React.FC = () => {
  ...
  const [senderName] = useState('Planet Keripik'); 
  ...
  useEffect(() => {
    ...
    // 2. Pre-fetch and convert logo to Base64 to avoid CORS issues in html2canvas
  ...
  const handleDownload = async () => {
    ...
    // Create a container for cloning to ensure exact width/layout capture
    ...
    try {
        ...
        // Force specific styles on the clone to ensure WYSIWYG
        clone.style.transform = 'none'; // Remove any preview scaling
        clone.style.width = '600px';    // Fixed width for sticker
        clone.style.height = 'auto';
        clone.style.aspectRatio = '1.5 / 1';
        clone.style.margin = '0';
        
        cloneContainer.appendChild(clone);

        // Allow images to decode
        ...
        const canvas = await html2canvas(clone, {
            ...
            windowWidth: 1200 // Larger window to prevent wrapping issues
        });

        const image = canvas.toDataURL('image/png');
        const link = document.createElement('a');
        link.href = image;
        link.download = `Stiker-${customerName.replace(/[^a-zA-Z0-9]/g, '-')}.png`;
    ...
    <div className="max-w-6xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 flex flex-col items-center">
             ...
             {/* STICKER CONTAINER - REPLICA SESUAI GAMBAR REFERENSI */}
             {/* Note: The previewRef must capture the STICKER itself, not the scrolling container */}
             <div className="bg-slate-200 p-8 rounded-2xl overflow-auto w-full flex justify-center shadow-inner">
                 ...
                 >
                     ...
                     <div className="text-center pt-2 pb-1">
                         <h1 className="text-[#dc2626] font-black text-6xl uppercase tracking-tighter scale-y-110" style={{ fontFamily: 'Arial, sans-serif' }}>
                             JANGAN DIBANTING
                         </h1>
                     </div>

                     {/* BODY CONTENT */}
                     <div className="flex-1 px-8 py-2 flex flex-col justify-center space-y-6">
                         {/* Field: Nama */}
                         <div className="flex items-end">
                             <span className="w-24 font-bold text-slate-600 text-xl">Nama :</span>
                             <div className="flex-1 border-b-2 border-slate-900 text-2xl font-bold text-slate-900 px-2 pb-1 leading-none">
                                 {customerName}
                             </div>
                         </div>
                         {/* Field: Alamat */}
                         <div className="flex items-start">
                             ...
                             <div className="flex-1 border-b-2 border-slate-900 text-lg font-bold text-slate-900 px-2 pb-1 leading-tight min-h-[3rem] break-words whitespace-pre-wrap">
                                 {address}
                             </div>
                         </div>
                         {/* Field: Telepon */}
                     ...
                     {/* FOOTER */}
                     <div className="mt-auto px-5 pb-2 pt-2 flex items-end relative w-full">
                         
                         {/* Left: Sender & Logo - Fixed Width for Balance */}
                         <div className="w-[150px] flex flex-col pb-3 shrink-0">
                             <span className="text-[10px] font-bold text-slate-500 mb-1">Pengirim:</span>
                             <div className="flex flex-col">
                                 {/* LOGO */}
                                 {logoBase64 && (
                                     <img src={logoBase64} className="h-12 w-auto object-contain self-start mb-1" alt="Logo" />
                                 )}
                                 <div className="flex flex-col leading-none">
                                     <span className="font-black text-slate-900 text-lg tracking-tighter">PLANET</span>
                                     <span className="font-black text-slate-900 text-lg tracking-tighter">KRIPIK!</span>
                                 </div>
                             </div>
                         </div>

                         {/* Middle: Address Box - Flex Grow to Center */}
                         <div className="flex-1 flex justify-center pb-5">
                             <div className="bg-[#d9480f] text-white px-3 py-2 text-[11px] font-bold leading-snug flex flex-col items-center justify-center text-center rounded-sm shadow-sm w-full max-w-[260px]">
                                 <span>Jalan Tempean IV Rt/Rw :</span>
                                 <span>04/06 Madyorenggo Talok</span>
                                 <span>Turen Malang ,082338247777</span>
                             </div>
                         </div>

                         {/* Right: QR Code & Website - Fixed Width for Balance */}
                         <div className="w-[150px] flex items-center justify-end gap-2 pb-4 shrink-0">
                             <img 
                             ...
                             <div className="flex flex-col leading-tight">
                                 ...
                                 <span className="text-[10px] font-bold text-slate-600">Scan sini</span>
                             </div>
                         </div>
                         
                         {/* Bottom URL Text */}
                         <div className="absolute bottom-1 left-0 w-full text-center">
                            <span className="text-[10px] font-bold text-slate-800 tracking-wide bg-white px-2">www.planetkeripik.com</span>
                         </div>
                     </div>
                 </div>
             </div>
